        ...
        <div className="collection-item col-sm-4 col-xs-6">
            <div className="item">
                ...
                <div className="info">
                    ...
                    <div className="button-wrapper">{isTotalInputEmptyOrZero ? <span></span> : <a className="shopping-cart" href="#" onClick={(e) => {props.onClickAdd(e, props.product)}}>Add to cart</a>}</div>
                    <div className="desc text-center"><span className="title">{isStockQuantityEmpty ? "" : props.product.quantity}</span>&nbsp; {props.product.quantity == 0 ? <span className="outOfStock">Out of Stock</span> : isItemSingular ? 'items in stock' : 'item in stock'}</div>
                </div>
            </div>
        </div>
    );
